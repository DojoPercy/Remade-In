...
export default function PartnerForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    body.append(FIELD_IDS.name,    data.get('name')    as string)
    body.append(FIELD_IDS.org,     data.get('org')     as string)
    body.append(FIELD_IDS.email,   data.get('email')   as string)
    body.append(FIELD_IDS.type,    data.get('type')    as string)
    body.append(FIELD_IDS.message, data.get('message') as string)

    try {
      // Google Forms doesn't support CORS — use no-cors so the browser
      // sends the request without expecting a readable response.
