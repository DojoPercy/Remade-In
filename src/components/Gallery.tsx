...
import { galleryImagesQuery, homePageQuery } from '@/lib/sanity/queries'
import type { GalleryImage, HomePage } from '@/lib/sanity/types'
import { imageUrl } from '@/lib/sanity/image'
import { colors, fonts } from '@/lib/tokens'
import SectionDivider from '@/components/ui/SectionDivider'
import GalleryStrip, { type CellData } from '@/components/GalleryStrip'

const GALLERY_TEXT_ACCENT = '#cbd183'
...
const FALLBACK: Record<string, CellData> = {
  ...
  d: { src: '/Events/EDIT X STUDIO BAYBEE-8940.jpg', alt: 'Studio workshop - Edit x Studio Baybee', location: 'The Netherlands', position: '50% center' },
...
export default async function Gallery() {
  let cells = FALLBACK
  let galleryCopy: Pick<HomePage, 'galleryEyebrow' | 'galleryHeadline' | 'galleryAccent'> | null = null

  try {
    galleryCopy = await client.fetch<Pick<HomePage, 'galleryEyebrow' | 'galleryHeadline' | 'galleryAccent'> | null>(
      homePageQuery,
      {},
  ...
  const eyebrow = galleryCopy?.galleryEyebrow ?? 'In The Field'
  const headline = galleryCopy?.galleryHeadline ?? 'People, places'
  const accent = galleryCopy?.galleryAccent ?? '& real moments.'
